...
export async function getEstimatedHotelPrice(
  ...
  try {
    ...
    if (pricePerNight === 0) {
      ...
      const basePrice = mockBasePriceByStar[stars] || 2000;
      // Fluctuation +- 10%
      const fluctuation = 1 + ((Math.random() * 0.2) - 0.1); 
      
      pricePerNight = Math.round(basePrice * multiplier * fluctuation);
    ...
    const fallbackPrice = (mockBasePriceByStar[stars] || 2000) * nights * rooms * 1.07 + 300;
    return {
      pricePerNight: mockBasePriceByStar[stars] || 2000,
      nights,
      rooms,
      totalCost: Math.round(fallbackPrice),
      currency: "THB",
      source: 'FALLBACK',
