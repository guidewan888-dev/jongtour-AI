    ...
    if (initialMessage.tool_calls && initialMessage.tool_calls.length > 0) {
      ...
      for (const toolCall of initialMessage.tool_calls) {
        ...
        if (toolCall.function.name === "search_wholesale_tours") {
          ...
          const fitPrompt = `You are an expert travel agent. Generate a detailed day-by-day itinerary for a private tour to ${args.country} for ${args.durationDays} days. You MUST write the entire itinerary in THAI language. Return ONLY JSON matching this format: { "title": "Trip Name", "marketingHeadline": "A catchy promotional Thai headline for this trip", "airlineCode": "2-letter IATA airline code suitable for this trip (e.g. TG, XJ, EK, SQ)", "coverImagePrompt": "A short English phrase describing a stunning wide landscape cover image for this trip", "estimatedPrice": "${pricingData.sellingPricePerPax.toLocaleString()} THB/ท่าน", "days": [{ "day": 1, "title": "Day Title", "detail": "Day details", "meals": { "breakfast": true, "lunch": false, "dinner": true }, "hotel": "Hotel Name or -", "imagePrompt": "A short English phrase describing the main attraction" }] }`;
