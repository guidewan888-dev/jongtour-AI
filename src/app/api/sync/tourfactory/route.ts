...
const TOURFACTORY_API_URL = "https://api.tourfactory.co.th/v1/programtours";
...
export async function GET(request: Request) {
  ...
  try {
    // 1. Fetch data from Tour Factory API
    const response = await fetch(TOURFACTORY_API_URL, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      throw new Error(`Tour Factory API returned status: ${response.status}`);
    }

    const toursData = await response.json();
    
    if (!Array.isArray(toursData) || toursData.length === 0) {
      return NextResponse.json({ message: "No tours found in Tour Factory API response" });
    }
    ...
    for (const cTour of toursData) {
      ...
      const tourId = `tourfac_tour_${cTour.id}`;
      ...
      if (Array.isArray(cTour.periods)) {
        for (const cPeriod of cTour.periods) {
          ...
          const departureId = `tourfac_period_${cPeriod.id}`;
    ...
    return NextResponse.json({
      success: true,
      message: "Tour Factory Sync completed successfully",
      data: {
    ...
    console.error("Tour Factory Sync Error:", error);
