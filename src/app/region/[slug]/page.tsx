...
const REGION_CONFIG: Record<string, any> = {
  europe: {
    name: "ยุโรป",
    image: "https://images.unsplash.com/photo-1499856871958-5b9627545d1a",
    countries: ["france", "switzerland", "italy", "uk", "germany", "austria", "czech"],
    countryNames: ["ฝรั่งเศส", "สวิตเซอร์แลนด์", "อิตาลี", "อังกฤษ", "เยอรมนี", "ออสเตรีย", "เช็ก"],
    seasons: [
      { name: "☀️ ฤดูร้อน (ซัมเมอร์)", desc: "มิถุนายน - สิงหาคม (อากาศอบอุ่น ฟ้าโปร่ง)" },
      { name: "🍂 ใบไม้เปลี่ยนสี", desc: "กันยายน - พฤศจิกายน" },
      { name: "⛄ หิมะ / ตลาดคริสต์มาส", desc: "ธันวาคม - กุมภาพันธ์" },
      { name: "🌷 ฤดูใบไม้ผลิ", desc: "มีนาคม - พฤษภาคม" }
    ],
    visa: "ส่วนใหญ่ต้องใช้ เชงเก้นวีซ่า (Schengen Visa) ยกเว้นประเทศที่อยู่นอกกลุ่ม เช่น อังกฤษ จอร์เจีย ตุรกี"
  },
  asia: {
    name: "เอเชีย",
    image: "https://images.unsplash.com/photo-1464817739973-0128fe77aaa1",
    countries: ["japan", "china", "korea", "taiwan", "hongkong", "singapore"],
    countryNames: ["ญี่ปุ่น", "จีน", "เกาหลี", "ไต้หวัน", "ฮ่องกง", "สิงคโปร์"],
    seasons: [
      { name: "🌸 ฤดูดอกไม้ร่วง", desc: "มีนาคม - พฤษภาคม" },
...
const DEFAULT_CONFIG = {
  ...
  seasons: [
    { name: "☀️ High Season", desc: "กรุณาสอบถามเจ้าหน้าที่" },
    { name: "❄️ Low Season", desc: "ราคาพิเศษ คนน้อย" }
  ],
  visa: "โปรดตรวจสอบรายละเอียดในแต่ละโปรแกรม"
};

export async function generateMetadata({ params }: { params: { slug: string } }) {
  const config = REGION_CONFIG[params.slug] || { name: params.slug };
  return {
...
export default async function RegionPage({ params }: { params: { slug: string } }) {
  ...
  const config = REGION_CONFIG[params.slug] || { ...DEFAULT_CONFIG, name: params.slug };
  const thRegionName = config.name;

  // Keyword search for region (often explicitly mentioned in tourName like "ทัวร์ยุโรป")
  ...
  // If not found by exact region name, fallback to searching by major countries
  if (tours.length === 0 && config.countryNames.length > 0) {
     const countryFilters = config.countryNames.map((c: string) => `tourName.ilike.%${c}%`).join(',');
  ...
  return (
    <main className="min-h-screen bg-background pb-20 font-sans">
      ...
      <div className="relative pt-24 pb-32 overflow-hidden bg-trust-900">
        ...
        <div className="relative max-w-7xl mx-auto px-4 z-10 flex flex-col items-center text-center mt-10">
          <div className="flex items-center gap-2 text-trust-300 text-xs md:text-sm font-bold tracking-wider mb-6 bg-trust-900/50 px-4 py-1.5 rounded-full border border-trust-700/50 backdrop-blur-md">
            ...
            <ChevronRight className="w-3.5 h-3.5" />
            <span className="text-white">ทัวร์{thRegionName}</span>
          </div>

          <Badge variant="brand" className="mb-6 px-4 py-1.5 text-sm">ภูมิภาคยอดนิยม 2026</Badge>
          <h1 className="text-4xl md:text-6xl font-black text-white tracking-tight mb-6 drop-shadow-lg">
            ทัวร์<span className="text-primary">{thRegionName}</span> คุ้มที่สุด
          </h1>
          <p className="text-lg text-trust-200 max-w-2xl mb-10 font-medium">
            ค้นหาแพ็กเกจทัวร์{thRegionName} อัปเดตล่าสุด เปรียบเทียบราคาจากโฮลเซลล์ชั้นนำ จองง่าย ปลอดภัย 100%
          </p>
          ...
          <div className="w-full max-w-4xl mt-10">
             <AiSearchBar placeholder={`AI ช่วยหาทัวร์${thRegionName} เช่น "ไปยุโรปตะวันออกเดือนหน้า ราคาไม่เกิน 50,000"`} defaultContext={{ region: params.slug }} />
          </div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 -mt-8 relative z-20">
         ...
         {/* Popular Countries in Region */}
         {config.countries.length > 0 && (
           <div className="mb-16">
              <h2 className="text-2xl font-black text-trust-900 mb-6 text-center">ประเทศยอดนิยมใน{thRegionName}</h2>
              <div className="flex flex-wrap justify-center gap-4">
                 {config.countries.map((countryEn: string, idx: number) => (
                    <Link key={countryEn} href={`/country/${countryEn}`} className="w-40 h-32 md:w-48 md:h-36">
                       <Card className="overflow-hidden hover:border-primary transition-all group shadow-sm border-border cursor-pointer h-full">
                          <div className="h-full bg-muted relative overflow-hidden flex items-center justify-center p-4">
                             <img src={config.image} className="absolute inset-0 w-full h-full object-cover opacity-20 group-hover:scale-110 transition-transform duration-500" />
                             <div className="absolute inset-0 bg-white/60 group-hover:bg-white/10 transition-colors"></div>
                             <h3 className="relative z-10 text-trust-900 font-black text-xl group-hover:text-white group-hover:drop-shadow-md transition-colors">{config.countryNames[idx]}</h3>
                          </div>
                       </Card>
                    </Link>
                 ))}
         ...
         <div className="mb-16 bg-muted/30 rounded-3xl p-8 border border-border">
            <h2 className="text-2xl font-black text-trust-900 mb-6 flex items-center gap-2"><CloudSun className="w-6 h-6 text-primary" /> เที่ยว{thRegionName}ช่วงไหนดี?</h2>
         ...
         <div id="tours" className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 pt-10">
            <h2 className="text-2xl font-black text-trust-900">แพ็กเกจทัวร์{thRegionName}ทั้งหมด ({formattedTours.length})</h2>
         ...
         {formattedTours.length === 0 ? (
            <div className="text-center py-16 px-4 bg-white rounded-3xl border border-border shadow-sm max-w-4xl mx-auto">
               ...
               <h3 className="text-2xl font-black text-trust-900 mb-3">ยังไม่มีโปรแกรมทัวร์{thRegionName} ในขณะนี้</h3>
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left max-w-3xl mx-auto">
                  ...
                  <Card className="border-border shadow-sm hover:border-trust-400 transition-colors bg-muted/30">
                     <CardContent className="p-6">
                        ...
                        <p className="text-sm text-muted-foreground mb-4">ทิ้งเบอร์ติดต่อหรือ Line ID ไว้ แอดมินจะรีบแจ้งทันทีที่มีทัวร์{thRegionName}อัปเดตใหม่</p>
         ...
         <div className="mt-10 mb-10 bg-trust-900 rounded-3xl p-10 text-center text-white overflow-hidden relative shadow-2xl">
            ...
            <h2 className="text-3xl font-black mb-4 relative z-10">ต้องการจัดกรุ๊ปทัวร์ส่วนตัวใน{thRegionName}?</h2>
