              ...
              <div className={`max-w-[80%] ${msg.role === "user" ? "items-end" : "items-start"} flex flex-col gap-3`}>
                ...
                {msg.tours && msg.tours.length > 0 && (
                  <div className="flex gap-4 overflow-x-auto pb-4 w-full max-w-[600px] snap-x">
                    {msg.tours.map(tour => (
                      <Link href={`/tour/${tour.id}`} key={tour.id} className="min-w-[300px] bg-white border border-gray-200 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow group snap-start block shrink-0">
                        <div className="relative aspect-[4/3] bg-gray-50 flex items-center justify-center overflow-hidden border-b border-gray-100">
                          <img src={tour.imageUrl || ''} alt={tour.title} className="w-full h-full object-cover object-top group-hover:scale-105 transition-transform duration-500" />
