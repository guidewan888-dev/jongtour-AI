      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <Link href="/account/payments" className="text-orange-500 hover:text-orange-600 text-sm font-bold flex items-center gap-1 mb-2 transition-colors">
            ...
            กลับหน้ารวมการชำระเงิน
          </Link>
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="text-3xl font-black text-slate-800">Payment {payment.paymentRef}</h1>
            {payment.status === 'COMPLETED' ? (
              <span className="bg-emerald-100 text-emerald-700 px-3 py-1.5 rounded-lg text-xs font-black tracking-wider uppercase border border-emerald-200">สำเร็จ (Paid)</span>
            ) : payment.status === 'PENDING' ? (
              <span className="bg-orange-100 text-orange-700 px-3 py-1.5 rounded-lg text-xs font-black tracking-wider uppercase border border-orange-200">รอชำระเงิน</span>
            ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-6 md:p-8">
            <h2 className="text-xl font-black text-slate-800 mb-6">รายละเอียดรายการ (Details)</h2>
            
            <div className="space-y-4">
              <div className="flex justify-between items-center py-3 border-b border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">อ้างอิงการจอง (Booking Ref)</span>
              ...
              <div className="flex justify-between items-center py-3 border-b border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">ชื่อทัวร์ (Tour Name)</span>
                <span className="text-sm font-bold text-slate-800 text-right max-w-[200px] truncate">{booking.tour.tourName}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">ช่องทางชำระเงิน (Method)</span>
                <span className="text-sm font-black text-slate-800 uppercase">{payment.paymentMethod.replace('_', ' ')}</span>
              </div>
              <div className="flex justify-between items-center py-3 border-b border-slate-100">
                <span className="text-xs font-black text-slate-400 uppercase tracking-wider">วันที่ทำรายการ (Date)</span>
                <span className="text-sm font-bold text-slate-800">{new Date(payment.createdAt).toLocaleDateString('th-TH')}</span>
              </div>
              <div className="flex justify-between items-center pt-3">
                <span className="text-sm font-black text-slate-800">ยอดสุทธิ (Total Amount)</span>
                <span className="text-2xl font-black text-orange-600">฿{payment.amount.toLocaleString()}</span>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
             <div className="p-5 border-b border-slate-100">
                <h3 className="font-black text-slate-800">ต้องการใบเสร็จรับเงิน?</h3>
             </div>
             <div className="p-5">
               {payment.status === 'COMPLETED' ? (
                 <Link href="/account/invoices" className="w-full bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 px-4 py-3 rounded-xl text-sm font-bold transition-colors flex items-center justify-center gap-2">
                   ...
                   ไปที่หน้า Invoice / Receipt
                 </Link>
               ) : (
                 <p className="text-sm text-slate-500 font-medium text-center">ใบเสร็จรับเงินจะดาวน์โหลดได้เมื่อการชำระเงินเสร็จสมบูรณ์</p>
               )}
             </div>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden p-6 md:p-8">
            <h2 className="text-xl font-black text-slate-800 mb-2">อัปโหลดสลิป (Upload Slip)</h2>
            <p className="text-sm text-slate-500 font-medium mb-6">กรุณาอัปโหลดหลักฐานการโอนเงินเพื่อยืนยันการชำระเงิน</p>
          ...
          <div className="bg-indigo-50 rounded-3xl border border-indigo-100 shadow-sm overflow-hidden p-6">
             <div className="flex gap-4">
                ...
                <div>
                  <h3 className="font-black text-indigo-900 mb-1">ติดต่อฝ่ายการเงิน</h3>
                  <p className="text-xs text-indigo-700 font-medium mb-3">หากมีปัญหาการชำระเงิน สามารถติดต่อแอดมินได้ทันที</p>
