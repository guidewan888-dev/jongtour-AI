...
export default async function CustomerPrivateGroupsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch(status) {
      case 'PENDING_SALE': return <span className="bg-orange-100 text-orange-700 px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider uppercase border border-orange-200">รอเจ้าหน้าที่ติดต่อกลับ (Pending Sale)</span>
      case 'QUOTED': return <span className="bg-indigo-100 text-indigo-700 px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider uppercase border border-indigo-200">ส่งใบเสนอราคาแล้ว (Quoted)</span>
      case 'REJECTED': return <span className="bg-slate-100 text-slate-600 px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider uppercase border border-slate-200">ยกเลิก (Cancelled)</span>
      case 'BOOKED': return <span className="bg-emerald-100 text-emerald-700 px-2.5 py-1 rounded-md text-[10px] font-black tracking-wider uppercase border border-emerald-200">ยืนยันการจองแล้ว (Booked)</span>
  ...
  return (
    <div className="space-y-6 animate-fade-in-up">
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 pb-4 border-b border-slate-200">
        <div>
          <h1 className="text-2xl font-black text-slate-800">คำขอกรุ๊ปส่วนตัว (Private Group Requests)</h1>
          <p className="text-slate-500 text-sm mt-1">ประวัติการขอออกแบบแพ็กเกจทัวร์ส่วนตัวโดย AI Planner</p>
        </div>
      ...
      {requests.length > 0 ? (
            ...
            <div key={req.id} className="bg-white border border-slate-200 rounded-2xl p-5 sm:p-6 shadow-sm hover:border-indigo-300 transition-colors flex flex-col md:flex-row items-start md:items-center justify-between gap-6 group">
              <div className="flex-1">
                ...
                <Link href={`/account/private-groups/${req.id}`} className="font-black text-slate-800 text-lg hover:text-indigo-600 transition-colors line-clamp-1">
                  ทัวร์ส่วนตัว {req.destination} ({req.durationDays} วัน)
                </Link>
                <div className="flex flex-wrap gap-x-6 gap-y-2 mt-3 text-sm font-medium text-slate-500">
                  ...
                  <span className="flex items-center gap-1.5">
                    ...
                    งบประมาณ {req.budgetPerPax ? `฿${req.budgetPerPax.toLocaleString()} / ท่าน` : 'ไม่ได้ระบุ'}
                  </span>
                  <span className="flex items-center gap-1.5">
              ...
              <div className="flex flex-col sm:flex-row items-center gap-4 w-full md:w-auto border-t md:border-t-0 md:border-l border-slate-100 pt-4 md:pt-0 md:pl-6 shrink-0">
                <div className="text-left sm:text-right w-full sm:w-auto">
                  <p className="text-[10px] font-black uppercase tracking-wider text-slate-400">ราคาประเมินโดย AI (Estimate)</p>
                  <p className="text-2xl font-black text-slate-800">฿{req.estimatedPrice.toLocaleString()}</p>
                </div>
        ...
        <div className="bg-white border border-slate-200 rounded-3xl p-12 text-center shadow-sm flex flex-col items-center">
          ...
          <h3 className="text-xl font-black text-slate-800 mb-2">ยังไม่มีคำขอกรุ๊ปส่วนตัว</h3>
          <p className="text-slate-500 font-medium mb-8 max-w-sm mx-auto">
            กำลังวางแผนทริปส่วนตัวสำหรับครอบครัวหรือองค์กรอยู่ใช่ไหม? ให้ AI Planner ช่วยออกแบบทริปและประเมินราคาให้คุณฟรี!
          </p>
