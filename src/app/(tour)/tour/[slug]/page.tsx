...
interface Dep { id:string; wholesaleId?:string; wholesaleName?:string; wholesaleSourceType?:string; startDate:string; endDate:string; priceAdult:number; priceChild:number; priceSingle:number; priceInfant:number; deposit:number; totalSeats?:number|null; booked?:number|null; remainingSeats:number; status:string; bus:string; }
interface Flight { route:string; flightNo:string; departure:string; arrival:string; airline:string; }
interface TourWholesaler { id:string; name:string; sourceType?:string; sourceTourCode?:string; sourceUrl?:string; pdfUrl?:string; }
interface Tour { id:string; slug:string; code:string; title:string; supplier:{id:string;name:string}; wholesalers?:TourWholesaler[]; country:string; city:string; duration:{days:number;nights:number}; images:string[]; price:{starting:number}; summary:string; highlights:string[]; pdfUrl?:string; flight?:{airline:string}; flights?:Flight[]; departures:Dep[]; included:string[]; excluded:string[]; deal?:{ type:'fire'|'discount'|null; daysLeft:number|null; currentPrice:number; minPrice:number; maxPrice:number; discountPercent:number; }; }
...
export default function TourDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/tours/${params.slug}`).then(r=>r.json()).then(d=>{
      if(d.tour) {
        setTour(d.tour);
        setSelectedWholesalerId(d.tour?.wholesalers?.[0]?.id || d.tour?.supplier?.id || '');
        fetchRecs(d.tour.country, d.tour.slug);
      }
  ...
  const selectedWholesaler = (tour.wholesalers || []).find(w => w.id === selectedWholesalerId);
  const filteredDepartures = selectedWholesalerId
    ? tour.departures.filter(d => (d.wholesaleId || tour.supplier.id) === selectedWholesalerId)
    : tour.departures;
  const availableDeps = filteredDepartures.filter(d => d.remainingSeats > 0);
  ...
  const seatValue = (n:number|null|undefined) => (n === null || n === undefined || !Number.isFinite(Number(n)) ? '-' : Number(n).toLocaleString());
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-col lg:flex-row gap-5">
            ...
            <div className="lg:w-[55%] flex flex-col min-w-0">
              ...
              {(tour.wholesalers || []).length > 0 && (
                <div className="mb-3">
                  <label className="text-xs text-slate-500 block mb-1">เลือกโฮลเซล</label>
                  <select
                    value={selectedWholesalerId}
                    onChange={(event) => { setSelectedWholesalerId(event.target.value); setShowAll(false); }}
                    className="border border-slate-300 rounded-lg px-3 py-2 text-sm min-w-[260px] max-w-full"
                  >
                    {(tour.wholesalers || []).map((wholesaler) => (
              ...
              <div className="flex flex-wrap items-center gap-3 mt-auto">
                {(selectedWholesaler?.pdfUrl || tour.pdfUrl) ? <a href={selectedWholesaler?.pdfUrl || tour.pdfUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-5 py-3 bg-blue-50 border-2 border-blue-200 text-blue-700 rounded-xl font-bold text-sm hover:bg-blue-100 transition-colors shadow-sm">Download PDF</a> : <span className="px-4 py-2.5 rounded-xl bg-slate-100 text-slate-500 text-xs font-bold">ยังไม่มีไฟล์ PDF</span>}
                <Link href={`/book/tour/${tour.slug}${selectedWholesalerId ? `?wholesale=${encodeURIComponent(selectedWholesalerId)}` : ''}`} className="px-6 py-3 rounded-xl bg-gradient-to-r from-primary-600 to-orange-500 text-white font-bold text-sm shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all">จองเลย →</Link>
          ...
          {availableDeps.length > 0 ? <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-slate-100">
                  {deps.map(d => <tr key={d.id} className="hover:bg-slate-50/50 transition-colors">
                    ...
                    <td className="px-3 py-3 text-center">{seatValue(d.totalSeats)}</td>
                    <td className="px-3 py-3 text-center">{seatValue(d.booked)}</td>
                    ...
                    <td className="px-3 py-3 text-center"><Link href={`/book/tour/${tour.slug}?dep=${d.id}${selectedWholesalerId ? `&wholesale=${encodeURIComponent(selectedWholesalerId)}` : ''}`} className="text-xs bg-primary-600 text-white px-3 py-1 rounded-full font-bold hover:bg-primary-700">จอง</Link></td>
                  </tr>)}
                </tbody>
              </table>
            </div>
            <div className="md:hidden px-4 pb-2 space-y-2">
              {deps.map(d => <div key={d.id} className="border border-slate-100 rounded-lg p-3">
                ...
                <div className="grid grid-cols-2 gap-1 text-xs text-slate-500"><span>ราคา: <b className="text-slate-800">{price(d.priceAdult)}</b></span><span>เด็ก: <b>{price(d.priceChild)}</b></span><span>พักเดี่ยว: <b>{price(d.priceSingle)}</b></span><span>มัดจำ: <b className="text-orange-600">{price(d.deposit)}</b></span><span>ที่นั่ง: <b>{seatValue(d.totalSeats)}</b></span><span>จอง: <b>{seatValue(d.booked)}</b></span><span>คงเหลือ: <b>{seatValue(d.remainingSeats)}</b></span></div>
              </div>)}
            </div>
