...
interface TourData {
  ...
  images: string[]; price: { starting: number };
  status: string; summary: string; highlights: string[];
  included: string[]; excluded: string[];
  policies: { payment: string; cancellation: string };
  pdfUrl?: string;
  itinerary: { day: number; title: string; description: string; meals: { breakfast: boolean; lunch: boolean; dinner: boolean } }[];
  departures: { id: string; startDate: string; endDate: string; priceAdult: number; priceChild: number; priceSingle: number; status: string; remainingSeats: number }[];
...
export default function TourDetailPage({ params }: { params: { slug: string } }) {
  ...
  useEffect(() => {
    fetch(`/api/tours/${params.slug}`)
      .then(r => r.json())
      .then(d => { if (d.tour) setTour(d.tour); else setError(d.error || 'ไม่พบทัวร์'); })
  ...
  if (error || !tour) return <div className="max-w-7xl mx-auto px-4 py-20 text-center"><div className="text-5xl mb-4">🔍</div><h1 className="text-xl font-bold mb-2">ไม่พบโปรแกรมทัวร์</h1><p className="text-slate-500 mb-6 text-sm">{error}</p><Link href="/search" className="btn-primary">กลับหน้าค้นหา</Link></div>;

  const rawAirline = tour.flight?.airline;
  const airlineName = (rawAirline && rawAirline !== 'ตามโปรแกรมทัวร์') ? rawAirline : null;
  const airlineLogo = airlineName ? AIRLINE_LOGOS[airlineName.toUpperCase()] || '' : '';
    ...
    <div className="bg-slate-50 pb-28 md:pb-8">
      ...
      <div className="bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 py-2.5 text-xs text-slate-400 font-medium flex items-center gap-1.5">
          ...
          <Link href="/search" className="hover:text-primary-600">ค้นหาทัวร์</Link><Ch />
      ...
      <div className="bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <div className="flex flex-col lg:flex-row gap-5">
            ...
            {/* IMAGE */}
            <div className="lg:w-[38%] rounded-xl overflow-hidden bg-slate-100 relative flex-shrink-0" style={{ minHeight: 260 }}>
              ...
              {airlineName && (
                <div className="absolute bottom-2 left-2 bg-white/90 backdrop-blur-sm text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1.5">
                  {airlineLogo ? <img src={airlineLogo} alt="" className="h-3.5 w-auto" onError={e => { (e.target as HTMLImageElement).style.display='none'; }} /> : <span>✈️</span>}
            ...
            <div className="lg:w-[37%] flex flex-col min-w-0">
              <div className="flex items-center gap-2 text-xs text-slate-400 mb-1">
                <span>🌍 {tour.country}</span><span>·</span><span>รหัสทัวร์ : {tour.code}</span>
              </div>
              ...
              {airlineName && (
                <div className="flex items-center gap-2.5 p-2 bg-blue-50/80 rounded-lg border border-blue-100 mb-3">
                  <div className="w-8 h-8 bg-white rounded-full flex items-center justify-center shadow-sm border border-blue-100 shrink-0 overflow-hidden">
                    {airlineLogo ? <img src={airlineLogo} alt="" className="w-5 h-5 object-contain" onError={e => { (e.target as HTMLImageElement).outerHTML='<span class="text-sm">✈️</span>'; }} /> : <span className="text-sm">✈️</span>}
                  </div>
                  <div><div className="font-bold text-slate-800 text-xs">{airlineName}</div><div className="text-[10px] text-slate-400">บริการอาหารบนเครื่อง</div></div>
                </div>
              )}
              {tour.highlights && tour.highlights.length > 0 && (
                <div>
                  <h3 className="text-xs font-bold text-slate-700 mb-1">⭐ ไฮไลท์ทัวร์</h3>
                  <p className="text-[11px] text-slate-600 leading-relaxed bg-amber-50/50 rounded-lg p-2 border border-amber-100/50">
            ...
            <div className="lg:w-[25%] flex-shrink-0">
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="bg-gradient-to-r from-primary-600 to-orange-500 px-4 py-3 text-white">
                  <p className="text-[10px] text-white/70">จองทัวร์ · ราคาเริ่มต้น</p>
                  {tour.price.starting > 0
                    ? <div className="flex items-baseline gap-1"><span className="text-2xl font-black">{tour.price.starting.toLocaleString()}</span><span className="text-xs">บาท/ท่าน</span></div>
                    : <p className="text-lg font-bold">สอบถามราคา</p>}
                </div>
                <div className="p-3 space-y-1 text-[11px]">
                  <IR l="รหัสทัวร์" v={tour.code} /><IR l="ระยะเวลา" v={`${tour.duration.days}วัน ${tour.duration.nights}คืน`} /><IR l="ประเทศ" v={tour.country} /><IR l="Wholesale" v={tour.supplier.name} />
                  {airlineName && <IR l="สายการบิน" v={airlineName} />}
                  {tour.pdfUrl && <a href={tour.pdfUrl} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 w-full py-2 rounded-lg border border-dashed border-primary-200 text-primary-600 font-bold text-[11px] hover:bg-primary-50 transition-colors mt-2">📥 ดาวน์โหลดโปรแกรมทัวร์</a>}
                  <Link href={`/book/tour/${tour.slug}`} className="block w-full py-2.5 rounded-lg bg-gradient-to-r from-primary-600 to-orange-500 text-white text-center font-bold text-sm shadow hover:shadow-lg transition-all mt-1.5">จองเลย →</Link>
                  <a href={LINE_URL} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center gap-1.5 w-full py-2 rounded-lg bg-emerald-500 text-white font-bold text-[11px] hover:bg-emerald-600 transition-colors shadow-sm">📱 สอบถามเพิ่มเติม</a>
                </div>
              </div>
            </div>
          </div>
            ...
            <>
              ...
              <div className="hidden md:block overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr className="bg-slate-50 text-xs text-slate-500 border-y border-slate-100">
                    <th className="text-left px-5 py-2.5">เดินทาง</th>
                    <th className="text-right px-3 py-2.5">ผู้ใหญ่</th>
                  ...
                  <tbody className="divide-y divide-slate-50">
                    {visibleDeps.map(dep => (
                      <tr key={dep.id} className="hover:bg-slate-50/50 transition-colors">
                        <td className="px-5 py-3 font-medium text-slate-700">{fmtDate(dep.startDate)} – {fmtDate(dep.endDate)}</td>
                        <td className="px-3 py-3 text-right font-bold text-slate-900">{dep.priceAdult > 0 ? `฿${dep.priceAdult.toLocaleString()}` : '-'}</td>
                        <td className="px-3 py-3 text-right text-slate-600">{dep.priceChild > 0 ? `฿${dep.priceChild.toLocaleString()}` : '-'}</td>
                        <td className="px-3 py-3 text-right text-slate-600">{dep.priceSingle > 0 ? `฿${dep.priceSingle.toLocaleString()}` : '-'}</td>
                        <td className="px-3 py-3 text-center"><span className={`inline-block min-w-[28px] py-0.5 rounded-full text-xs font-bold ${dep.remainingSeats > 5 ? 'bg-emerald-100 text-emerald-700' : dep.remainingSeats > 0 ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-600'}`}>{dep.remainingSeats}</span></td>
                        <td className="px-3 py-3 text-center"><span className={`text-xs font-bold px-2 py-0.5 rounded-full ${dep.remainingSeats > 0 ? 'bg-emerald-100 text-emerald-700' : 'bg-red-100 text-red-600'}`}>{dep.remainingSeats > 0 ? 'ว่าง' : 'เต็ม'}</span></td>
                        <td className="px-3 py-3 text-center">{dep.remainingSeats > 0 ? <Link href={`/book/tour/${tour.slug}`} className="text-xs bg-primary-600 text-white px-3 py-1 rounded-full font-bold hover:bg-primary-700">จอง</Link> : <span className="text-xs text-slate-300">-</span>}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="md:hidden px-4 pb-2 space-y-2">
                {visibleDeps.map(dep => (
                  <div key={dep.id} className="border border-slate-100 rounded-lg p-3 flex items-center justify-between">
                    <div><div className="text-sm font-medium text-slate-700">{fmtDate(dep.startDate)} – {fmtDate(dep.endDate)}</div><div className="text-xs text-slate-500 mt-0.5">{dep.priceAdult > 0 ? `฿${dep.priceAdult.toLocaleString()}/ท่าน` : 'สอบถาม'}</div></div>
                    {dep.remainingSeats > 0 ? <span className="text-xs bg-orange-100 text-orange-700 px-2.5 py-1 rounded-full font-bold">เหลือ {dep.remainingSeats}</span> : <span className="text-xs bg-red-100 text-red-600 px-2.5 py-1 rounded-full font-bold">เต็ม</span>}
                  </div>
                ))}
              </div>
              {tour.departures.length > 4 && (
                <div className="px-5 pb-4">
                  <button onClick={() => setShowAllDeps(!showAllDeps)} className="text-xs text-primary-600 font-bold hover:underline flex items-center gap-1">
                    📅 {showAllDeps ? '▲ ย่อ' : `ดูวันอื่นเพิ่มเติม (${tour.departures.length - 4} รอบ)`}
                  </button>
                </div>
              )}
            </>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        <div className="bg-gradient-to-r from-emerald-500 to-emerald-600 rounded-xl p-5 text-white flex flex-col md:flex-row items-center gap-4">
          <div className="flex items-center gap-3 flex-1">
            ...
            <div><h3 className="text-base font-black">สอบถามเพิ่มเติมทาง LINE</h3><p className="text-emerald-100 text-[11px]">เช็คที่นั่ง โปรโมชั่น ขอใบเสนอราคา</p></div>
          </div>
...
function IR({ l, v }: { l: string; v: string }) {
  return <div className="flex justify-between py-1 border-b border-slate-50 last:border-0"><span className="text-slate-400">{l}</span><span className="font-bold text-slate-800 text-right">{v}</span></div>;
}
function Ch() {
