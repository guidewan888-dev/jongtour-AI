...
const FILE_INTENT_CLASSIFIER_PROMPT = `คุณคือ AI File Intent Classifier สำหรับระบบ Jongtour
...
async function main() {
  ...
  let template = await prisma.aiPromptTemplate.findUnique({
    where: { name: 'FILE_INTENT_CLASSIFIER' }
  });

  if (!template) {
    template = await prisma.aiPromptTemplate.create({
      data: { name: 'FILE_INTENT_CLASSIFIER', currentVersion: 1 }
    });
  } else {
  ...
  const version = await prisma.aiPromptVersion.create({
    data: {
      ...
      version: newVersion,
      content: FILE_INTENT_CLASSIFIER_PROMPT,
      approvedBy: null
    }
  });

  console.log('Successfully seeded FILE_INTENT_CLASSIFIER prompt to Database!');
}
